...
~~Currently, when a user posts a task in the dashboard, the frontend routes the transaction to a hardcoded string `0xEscrowContract`. We need this to invoke the actual `TaskEscrow` smart contract.~~ Done.

**Files to update:** 
* `packages/frontend/src/pages/Tasks.tsx`

**Tasks:**
- [x] Read the deployed contract address generated by Hardhat (`packages/contracts/deploy.ts` should output the address; store it in the frontend's `.env.local` or a config file).
- [x] Replace `to: '0xEscrowContract'` in `Tasks.tsx` with the actual `TaskEscrow` deployed address.
